...
in
{
  ...
  config = lib.mkIf cfg.enable {
    ...
    # Auto-mounting for removable media (optional)
    services.udisks2.enable = lib.mkDefault true;
    
    # Documentation
    environment.etc."nixos/filesystem-support.txt".text = ''
      Supported filesystems: ${lib.concatStringsSep ", " cfg.filesystems}
      Network shares: ${if cfg.enableNetworkShares then "enabled" else "disabled"}
      FUSE support: ${if cfg.enableFuse then "enabled" else "disabled"}
      ZFS support: ${if cfg.enableZfs then "enabled" else "disabled"}
