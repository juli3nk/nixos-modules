  ...
  config = lib.mkMerge [
    # Basic configuration
    {
      virtualisation.podman = {
        dockerCompat = config.myPodman.enableDockerCompat;
        dockerSocket.enable = config.myPodman.enableDockerCompat;
      ...
      users.users.${config.mySystem.user}.extraGroups = [ "podman" ];
    }
    ...
    (lib.mkIf config.myPodman.enableRootless {
